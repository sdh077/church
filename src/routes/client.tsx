...
const PRESERVED: any = import.meta.glob("/src/pages/client/(_app|404|403).tsx", {
  eager: true,
});
const ROUTES: any = import.meta.glob("/src/pages/client/**/[a-z[]*.tsx", {
  eager: true,
});

const preserved: any = Object.keys(PRESERVED).reduce((preserved, file) => {
  const key = file.replace(/\/src\/pages\/client\/|\.tsx$/g, "");
...
const routes = Object.keys(ROUTES)
  ...
  .map((route) => {
    const path = route
      .replace(/\/src\/pages\/client|index|\.tsx$/g, "")
      .replace(/\/$/, "")
      .replace(/\[\.{3}.+\]/, "*")
      .replace(/\[(.+)\]/, ":$1");
